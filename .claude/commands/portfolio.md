...
要点（スキルの手順が正）:
- 保有 CSV は引数のパス、省略時は `data/portfolio.csv`（列: code,shares,avg_cost,acquired_date,memo）。無ければテンプレート `analysis/templates/portfolio-example.csv` を案内する。`data/` は gitignore 対象なので保有情報はコミットされない。
