...
夜間フォーキャスト（`/overnight`）も同じ機械可読契約で平日夜に台帳を醸成できる（[`docs/overnight-forecast.md`](docs/overnight-forecast.md)）。さらに **MAPE-K 夜間セルフ改善**（`/mape-night` / `/mape-execute`）は夜間に2系統を回す: **(1) リポジトリ健全性**（pytest・knowledge 索引整合・テストの穴・TODO 等）と、**(2) 分析の答え合わせ（track record）**——夜間フォーキャストの方向的中率・Brier とリサーチジャーナルの hit 率・検証期日超過を継続測定し、**「分析→記録→答え合わせ→測定→手法改善」の閉ループ**を駆動する（弱ければ予想モデルの重み見直し等を提案）。改善案は GitHub 計画イシューに「📊 分析の答え合わせ」ダッシュボード＋リスク3分類チェックリストで掲示し、承認済みを1周1件だけ安全に実装（pytest 緑→ドラフト PR）。設計・運用は [`docs/mape-k.md`](docs/mape-k.md)。
