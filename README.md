...
flowchart TD
    ...
    KB["📚 ナレッジベース<br/>knowledge/（100文書）"] -.->|"解釈の枠組み"| SK
...
夜間フォーキャスト（`/overnight`）も同じ機械可読契約で平日夜に台帳を醸成できる（[`docs/overnight-forecast.md`](docs/overnight-forecast.md)）。さらに **MAPE-K 夜間セルフ改善**（`/mape-night` / `/mape-execute`）は**「株の解析の醸成」を主題**に、夜間に分析の2軸を回す: **(1) 予測精度**——夜間フォーキャストの方向的中率・Brier とリサーチジャーナルの hit 率・検証期日超過を測り、弱ければ予想モデルの重み・較正を改善へ回す。**(2) 分析カバレッジ**——ユニバース網羅・未分析銘柄・ナレッジの陳腐化を測り、分析資産を広げ・新鮮に保つ。こうして**「分析→記録→答え合わせ→測定→手法改善」の閉ループ**を駆動する（システム/リポジトリの健全性は主題ではなく、pytest を最小ガードレールとして測るのみ）。改善案は GitHub 計画イシューに「🎯予測精度」「🗺️分析カバレッジ」ダッシュボード＋リスク3分類チェックリストで掲示し、承認済みを1周1件だけ安全に実装（pytest 緑→ドラフト PR）。設計・運用は [`docs/mape-k.md`](docs/mape-k.md)。
...
|---|---|---|
| **Yahoo Finance（既定・主軸）** | 日足 OHLCV・基本ファンダ指標。**直近の株価で分析する通常用途はこれ**。実装は `requests` による**非公式エンドポイント直叩き**が第一手段で、失敗時のみ yfinance ライブラリにフォールバック | 非公式 API。日本株では分割・配当調整の不備や欠損が起きることが知られており、`auto_adjust=True` は配当込み調整のため他ソースの系列と一致しない。商用利用は規約上グレー。**重要な意思決定の前は他ソースと突き合わせる** |
...
### ⚠️ Yahoo Finance 経路の規約上の注意（利用者の責任で確認してください）

既定の価格・ファンダ取得（`analysis/stocklib/data.py`）は、**Yahoo Finance が公開 API として提供していないエンドポイント**（`query1/2.finance.yahoo.com` の chart / quoteSummary / fundamentals-timeseries）を `requests` で直接呼び出す。yfinance ライブラリと実質的に同じ経路だが、テンプレートとして配布している以上、事実を明記しておく。

- **非公式・無保証**: Yahoo はこれらを開発者向け API として提供しておらず、予告なく仕様変更・遮断されうる。
- **規約上のリスク**: Yahoo の利用規約は自動化された手段によるデータ取得や再配布を制限する条項を含むと解されており、個人利用・非商用であっても抵触しうるという論点がある。**本ツールの利用が規約に適合するかの判断は利用者自身の責任**。商用利用・再配布・サービス組み込みを検討する場合は必ず規約原文を確認すること。
...
**商標**: 「日経平均株価」「日経225」は株式会社日本経済新聞社の、「TOPIX」「東証株価指数」は株式会社JPX総研または日本取引所グループの、「J-Quants」は日本取引所グループの、「Yahoo! Finance」は Yahoo Inc. の商標または登録商標です。本リポジトリはこれらの商標権者と提携・後援関係にはなく、各名称は分析対象・データ提供元を識別するための説明的使用にとどまります。`analysis/universe/*.csv` の `sector` 列は JPX の33業種分類の名称に基づきます（分類体系の権利は JPX に帰属）。
